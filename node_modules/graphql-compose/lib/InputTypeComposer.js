...
function isComposeInputType(type) {
  return (0, _graphql.isInputType)(type) || Array.isArray(type) && isComposeInputType(type[0]) || type instanceof InputTypeComposer || type instanceof _EnumTypeComposer.EnumTypeComposer || type instanceof _ScalarTypeComposer.ScalarTypeComposer;
}
...
class InputTypeComposer {
  ...
  static createTemp(typeDef, schemaComposer) {
    ...
    if ((0, _is.isString)(typeDef)) {
      ...
      const type = new _graphql.GraphQLInputObjectType({
        ...
        fields: (0, _is.isFunction)(fields) ? () => (0, _configAsThunk.resolveInputConfigMapAsThunk)(sc, fields(), typeDef.name) : () => ({})
      });
      ITC = new InputTypeComposer(type, sc);
      if ((0, _is.isObject)(typeDef.fields)) ITC.addFields(typeDef.fields);
    ...
    Object.keys(newFields).forEach(fieldName => {
      ...
      if (names.length === 0) {
        ...
        if (!this.hasField(name)) {
          childTC = InputTypeComposer.createTemp(`${this.getTypeName()}${(0, _misc.upperFirst)(name)}`, this.schemaComposer);
  ...
  getFieldExtensions(fieldName) {
    ...
    if ((0, _is.isObject)(field) && !(0, _is.isFunction)(field) && !Array.isArray(field) && !isComposeInputType(field) && // Flow is misbehaving so I have to add this
    // it should be handled by the above isComposeInputType, but somehow is not
    !(field instanceof _graphql.GraphQLList) && !(field instanceof _graphql.GraphQLNonNull)) {
  ...
  setFieldExtensions(fieldName, extensions) {
    ...
    this.setField(fieldName, _objectSpread({}, fieldConfig, {
      extensions
    }));
    return this;
  }
