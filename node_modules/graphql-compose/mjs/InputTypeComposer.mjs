...
import { GraphQLInputObjectType, GraphQLNonNull, GraphQLList, getNamedType, isInputType } from './graphql';
...
export function isComposeInputType(type) {
  return isInputType(type) || Array.isArray(type) && isComposeInputType(type[0]) || type instanceof InputTypeComposer || type instanceof EnumTypeComposer || type instanceof ScalarTypeComposer;
}
export class InputTypeComposer {
  ...
  static createTemp(typeDef, schemaComposer) {
    ...
    if (isString(typeDef)) {
      ...
      const type = new GraphQLInputObjectType({
        ...
        fields: isFunction(fields) ? () => resolveInputConfigMapAsThunk(sc, fields(), typeDef.name) : () => ({})
      });
      ITC = new InputTypeComposer(type, sc);
    ...
    Object.keys(newFields).forEach(fieldName => {
      ...
      if (names.length === 0) {
        ...
        if (!this.hasField(name)) {
          childTC = InputTypeComposer.createTemp(`${this.getTypeName()}${upperFirst(name)}`, this.schemaComposer);
  ...
  getFieldExtensions(fieldName) {
    ...
    if (isObject(field) && !isFunction(field) && !Array.isArray(field) && !isComposeInputType(field) && // Flow is misbehaving so I have to add this
